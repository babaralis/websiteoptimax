    ...
    <section id="pricing" className="py-20 lg:py-28 relative overflow-hidden pt-0">
      <div className="container">
        <FadeIn className="text-center mb-12">
          <span className="inline-flex items-center gap-2 text-[#01E6DF] text-sm font-semibold tracking-wide uppercase mb-4 px-4 py-1.5 rounded-full bg-[#01E6DF]/10 border border-[#01E6DF]/20">
            Pricing & Packages
          </span>
          
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-display font-bold mb-4">
            Get More for Less with Our{" "}
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#01E6DF] to-[#0149D1]">Value-Packed Plans</span>
          </h2>
        </FadeIn>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          {packages.map((pkg, index) => (
            <FadeIn key={pkg.name} delay={index * 0.1}>
              <motion.div
                className={`glass-card-elevated p-6 flex flex-col relative ${pkg.popular ? 'ring-2 ring-[#01E6DF]' : ''}`}
                whileHover={{ y: -4 }}
                transition={{ duration: 0.3 }}
              >
                {pkg.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-[#01E6DF] text-[#0149D1] text-xs font-semibold">
                ...
                <div className="text-center mb-6">
                  ...
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <span className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[#01E6DF] to-[#0149D1]">{pkg.price}</span>
                  </div>
                </div>
                ...
                <ul className="space-y-3 mb-6 h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                  {pkg.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-2 text-sm text-muted-foreground">
                      <Check className="w-4 h-4 text-[#01E6DF] shrink-0 mt-0.5" />
                      <span>{feature}</span>
                    </li>
                  ))}
                </ul>
                ...
                <Button 
                  ...
                  className={`w-full gap-2 ${pkg.popular ? 'bg-[#01E6DF] text-[#0149D1] hover:bg-[#01E4DB] hover:border-[#01E4DB] shadow-[#01E6DF]/25 hover:shadow-[#01E4DB]/30' : 'border-[#01E6DF] text-[#01E6DF] hover:bg-[#01E6DF] hover:text-[#0149D1] hover:border-[#01E6DF]'}`}
        ...
        <FadeIn className="text-center mb-8">
          <h3 className="text-4xl font-display font-bold">Premium <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#01E6DF] to-[#0149D1]">Packages</span>
          </h3>
        </FadeIn>
          ...
          {premiumPackages.map((pkg, index) => (
            <FadeIn key={pkg.name} delay={index * 0.1}>
              ...
              >
                ...
                <Button 
                  variant="heroOutline" 
                  className="w-full gap-2 border-[#01E6DF] text-[#01E6DF] hover:bg-[#01E6DF] hover:text-[#0149D1] hover:border-[#01E6DF]"
                  onClick={() => handlePayment(pkg)}
                  disabled={loading !== null}
                >
                  {loading === generatePackageString(pkg) ? (
