...
export function WebDesignV4ConsultationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data = {
      ...
      title: "Consultation Form (Floating v4)",
    ...
    try {
      ...
      trackEvent({
        action: "form_submit",
        category: "Contact",
        label: "Consultation Form (Floating v4)",
      });
      ...
      trackEvent({
        action: "form_success",
        category: "Contact",
        label: "Consultation Form Completed v4",
      });
          ...
          <div className="flex-1 glass-card-premium rounded-l-lg md:rounded-l-2xl rounded-r-none p-4 sm:p-6 md:p-8 relative overflow-hidden">
            <div className="relative z-10">
              <h2 className="text-xl sm:text-2xl md:text-3xl font-display font-bold mb-4 sm:mb-6 text-foreground">
                Book a{" "}
                <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#14B8A6] to-[#84CC16]">Free</span> Consultation
              </h2>
              {isSubmitted ? (
